...
class HrPayslipRun(models.Model):
    ...
    def action_confirm(self):
        payslip_confirm_result = self.mapped('slip_ids').filtered(lambda slip: slip.state in ['verify']).action_confirm()
        self.write({'state' : 'confirm'})
        return payslip_confirm_result

    def action_approve(self):
        payslip_approve_result = self.mapped('slip_ids').filtered(lambda slip: slip.state in ['confirm']).action_approve()
    ...
    def action_refuse(self):
        payslip_refuse_result = self.mapped('slip_ids').filtered(lambda slip: slip.state not in ['done','paid','cancel','refuse']).action_refuse()
        self.write({'state' : 'refuse'})
        return payslip_refuse_result

    def action_cancel(self):
        payslip_cancel_result = self.mapped('slip_ids').filtered(lambda slip: slip.state not in ['done','paid','cancel','refuse']).action_payslip_cancel()
        self.write({'state' : 'cancel'})
        return payslip_cancel_result

    def action_draft(self):
        payslip_draft_result = self.mapped('slip_ids').filtered(lambda slip: slip.state not in ['done','paid']).action_payslip_draft()
        self.write({'state' : 'draft'})
        return payslip_draft_result

    def action_register_payment(self):
        # Register payment function
        # After that state should be changed to paid
